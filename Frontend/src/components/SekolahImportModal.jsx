...
export default function SekolahImportModal({ onClose, onImported }) {
  ...
  async function handleDownloadTemplate() {
    ...
    setError('')
    try {
      await api.downloadSekolahImportTemplate()
    } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    setResult(null)
    try {
      const res = await api.importSekolah(file)
      setResult(res)
...
<ModalCloseButton onClose={onClose} />
        <h2 className="text-lg font-bold text-navy mb-1">Import Data Sekolah</h2>
        <p className="text-sm text-navy/50 mb-4">
          Untuk memasukkan banyak sekolah sekaligus (mis. dataset satu provinsi) dari file Excel.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-xs text-navy/40">
            Maksimal 1000 baris per file. NPSN yang sudah terdaftar akan otomatis dilewati (tidak
            dibuat duplikat).
          </p>
