...
export default function CapaianPembelajaranImportModal({ onClose, onImported }) {
  ...
  async function handleDownloadTemplate() {
    ...
    setError('')
    try {
      await api.downloadCapaianPembelajaranTemplate()
    } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    setResult(null)
    try {
      const res = await api.importCapaianPembelajaran(file)
      setResult(res)
...
<ModalCloseButton onClose={onClose} />
        <h2 className="text-lg font-bold text-navy mb-1">Import Capaian Pembelajaran</h2>
        <p className="text-sm text-navy/50 mb-4">Untuk memasukkan banyak CP sekaligus dari file Excel.</p>
        ...
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 mb-4 space-y-2">
          ...
          <p className="text-xs text-navy/50">
            Mata Pelajaran dan Tahun Ajaran harus sudah terdaftar. Baris dengan kombinasi Tahun Ajaran + Mata
            Pelajaran + Fase + Elemen yang sudah ada akan dilewati (tidak menimpa).
          </p>
          <button
            type="button"
