...
const EVENT_LABEL = {
  ...
  applied: 'Diterapkan ke Jadwal',
}
const FIELD_LABEL = {
  jenis: 'Jenis',
  tanggal_perubahan: 'Tanggal Perubahan',
  tanggal_baru: 'Tanggal Baru',
  jadwal_baru: 'Jadwal Baru',
  guru_baru: 'Guru Baru',
  ruang_lama: 'Ruang Lama',
  ruang_baru: 'Ruang Baru',
  alasan: 'Alasan',
  status: 'Status',
  catatan: 'Catatan',
...
export default function PerubahanJadwalDetailModal({ id, onClose }) {
  const [detail, setDetail] = useState(null)
  const [error, setError] = useState('')

  useEffect(() => {
    api
      .getPerubahanJadwal(id)
      .then(setDetail)
        ...
        {detail && (
          <>
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {detail.diterapkan_at && <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-sky-100 text-sky-700">Sudah diterapkan ke jadwal</span>}
            </div>
            <p className="text-xs text-navy/50 mb-4 capitalize">
              Perubahan {detail.jenis} · {detail.jenis === 'sementara' ? `tanggal ${detail.tanggal_perubahan}${detail.tanggal_baru && detail.tanggal_baru !== detail.tanggal_perubahan ? ` dipindah ke ${detail.tanggal_baru}` : ''}` : `berlaku mulai ${detail.tanggal_perubahan}`}
            </p>

            <div className="grid sm:grid-cols-2 gap-3 mb-4">
              <Info label="Jadwal Lama" value={`${detail.jadwal_lama}${detail.ruang_lama ? ` · ${detail.ruang_lama}` : ''}`} />
              <Info label="Jadwal Baru" value={`${detail.jadwal_baru}${detail.ruang_baru ? ` · ${detail.ruang_baru}` : ''}`} />
              <Info label="Guru Lama" value={detail.guru_lama?.nama} />
              <Info label="Guru Baru" value={detail.guru_baru?.nama} />
              <Info label="Alasan" value={detail.alasan} />
              <Info label="Pengaju" value={detail.pengaju} />
              <Info label="Catatan" value={detail.catatan} />
              {detail.diputuskan_oleh && <Info label="Keputusan" value={`${detail.diputuskan_oleh}${detail.catatan_keputusan ? ` — ${detail.catatan_keputusan}` : ''}`} />}
            </div>
