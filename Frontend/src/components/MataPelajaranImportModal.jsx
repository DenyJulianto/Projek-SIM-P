...
export default function MataPelajaranImportModal({ onClose, onImported }) {
  ...
  async function handleDownloadTemplate() {
    ...
    setError('')
    try {
      await api.downloadMataPelajaranTemplate()
    } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    setResult(null)
    try {
      const res = await api.importMataPelajaran(file)
      setResult(res)
  ...
  return (
    <div className="fixed inset-0 bg-navy/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6">
        <h2 className="text-lg font-bold text-navy mb-1">Import Mata Pelajaran</h2>
        <p className="text-sm text-navy/50 mb-4">Untuk memasukkan banyak mata pelajaran sekaligus dari file Excel.</p>
        ...
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 mb-4 space-y-2">
          ...
          <p className="text-xs text-navy/50">
            Baris dengan <strong>Kode Mapel</strong> yang sudah terdaftar akan otomatis dilewati (tidak menimpa data yang ada).
          </p>
          <button
            type="button"
