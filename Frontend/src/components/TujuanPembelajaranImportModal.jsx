...
export default function TujuanPembelajaranImportModal({ onClose, onImported }) {
  ...
  async function handleDownloadTemplate() {
    ...
    setError('')
    try {
      await api.downloadTujuanPembelajaranTemplate()
    } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    setResult(null)
    try {
      const res = await api.importTujuanPembelajaran(file)
      setResult(res)
...
<ModalCloseButton onClose={onClose} />
        <h2 className="text-lg font-bold text-navy mb-1">Import Tujuan Pembelajaran</h2>
        <p className="text-sm text-navy/50 mb-4">Untuk memasukkan banyak TP sekaligus dari file Excel.</p>
        ...
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 mb-4 space-y-2">
          ...
          <p className="text-xs text-navy/50">
            CP tujuan (Tahun Ajaran + Mata Pelajaran + Fase + Elemen) harus sudah ada — tambahkan CP-nya terlebih
            dahulu lewat menu Capaian Pembelajaran. Baris dengan kombinasi CP + Tingkat + Semester + Nomor Urut
            yang sudah ada akan dilewati.
          </p>
          <button
            type="button"
