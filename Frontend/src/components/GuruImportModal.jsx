...
export default function GuruImportModal({ onClose, onImported }) {
  ...
  async function handleDownloadTemplate() {
    ...
    setError('')
    try {
      await api.downloadGuruImportTemplate()
    } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    setResult(null)
    try {
      const res = await api.importGuruDirectoryNasional(file)
      setResult(res)
...
<ModalCloseButton onClose={onClose} />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-xs text-navy/40">
            Maksimal 1000 baris per file. NIP/NUPTK yang sudah terdaftar di sekolah tujuan akan
            otomatis dilewati.
          </p>
