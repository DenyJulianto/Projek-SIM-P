          ...
          <div className="relative bg-gradient-to-br from-emerald-50 to-white border border-emerald-100 rounded-2xl p-6 sm:p-8 overflow-hidden">
            ...
            <div className="relative z-10">
              <p className="text-sm text-navy/50 flex items-center gap-1.5 mb-1">
                <span className="text-lg">👋</span> Selamat datang,
              </p>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-navy/10 p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <ShortcutTile icon={UsersIcon} label="Kelola Pengguna" tone="emerald" onClick={() => onNavigate('pengguna')} />
              <ShortcutTile icon={ShieldCheckIcon} label="Atur Role & Permission" tone="emerald" onClick={() => onNavigate('hak-akses')} />
              <ShortcutTile icon={GearIcon} label="Konfigurasi Sistem" tone="blue" onClick={() => onNavigate('konfigurasi')} />
              <ShortcutTile icon={LogIcon} label="Lihat Audit Log" tone="blue" onClick={() => onNavigate('audit-log')} />
            </div>
          </div>
          ...
          >
            <LeafDecoration className="absolute -right-4 -bottom-6 h-28 w-28 opacity-70" />
...
function ShortcutTile({ icon: Icon, label, tone, onClick }) {
  const toneClass =
    tone === 'emerald'
      ? 'bg-emerald-50 hover:bg-emerald-100 text-emerald-700'
      : 'bg-blue-50 hover:bg-blue-100 text-blue-700'
  return (
    <button onClick={onClick} className={`rounded-xl p-3.5 text-left transition-colors ${toneClass}`}>
  ...
  return (
    <button onClick={onClick} className="bg-white rounded-2xl border border-navy/10 p-5 text-left hover:border-navy/20 transition-colors">
      <div className="flex items-center justify-between mb-3">
        ...
        <ChevronIcon className="h-4 w-4 text-navy/25" />
      </div>
...
function LeafDecoration(props) {
  return (
    <svg {...props} viewBox="0 0 100 100" fill="none">
      <circle cx="50" cy="50" r="50" fill="#14a673" fillOpacity="0.06" />
      <path d="M30 70C30 45 50 30 75 30c0 25-20 45-45 40Z" fill="#14a673" fillOpacity="0.12" />
    </svg>
  )
}
