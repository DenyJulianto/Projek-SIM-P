...
// Profil profesional dipakai bersama oleh guru (tabel guru) dan staf non-guru
// seperti Bendahara (tabel users) — bentuk datanya sama, hanya endpoint-nya beda.
const GURU_CLIENT = {
  get: () => api.getMyGuruProfil(),
  update: (data) => api.updateMyGuruProfilProfesional(data),
  upload: (files) => api.uploadMyGuruSertifikat(files),
  remove: (id) => api.deleteMyGuruSertifikat(id),
  fileUrl: (item) => `${BASE_URL}/sertifikat-file/${item.path.replace(/^sertifikat\//, '')}`,
}

const STAFF_CLIENT = {
  get: () => api.getMyStaffProfil(),
  update: (data) => api.updateMyStaffProfil(data),
  upload: (files) => api.uploadMyStaffSertifikat(files),
  remove: (id) => api.deleteMyStaffSertifikat(id),
  fileUrl: (item) => `${BASE_URL}/sertifikat-staf-file/${item.path.replace(/^sertifikat-staf\//, '')}`,
}
...
export default function MyProfile({ onBack, guruProfile = false, staffProfile = false, kelas = null }) {
  ...
  const [tab, setTab] = useState(profesional ? 'profesional' : 'personal')
  const [guru, setGuru] = useState(null)

  useEffect(() => {
    if (!profesional) return
    client.get().then(setGuru).catch(() => setGuru(null))
  }, [profesional, client])

  const card = profesional
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="w-full md:w-64 shrink-0">
          <div className={`${card} p-6 text-center mb-4`}>
            ...
            <p className="font-bold text-navy mt-3">
              {profesional && guru ? [guru.nama, guru.gelar].filter(Boolean).join(', ') : user?.name}
            </p>
        ...
        <div className={`flex-1 ${card} p-6`}>
          {tab === 'kompetensi' ? (
            <KompetensiSertifikatForm guru={guru} client={client} onSaved={setGuru} />
          ) : tab === 'profesional' ? (
            <ProfilProfesionalForm
              user={user}
              guru={guru}
              kelas={kelas}
              client={client}
...
function ProfilProfesionalForm({ user, guru, kelas, client, staff, onSaved, onUserSaved }) {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-navy/45 mb-5">
        {staff ? 'Ditampilkan sebagai profil staf sekolah.' : 'Ditampilkan sebagai profil guru kepada orang tua dan siswa.'}
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {!staff && (
          <ProfSection title="Tentang Saya">
            <Field label={staff ? 'Kata Mutiara / Kutipan' : 'Kata Mutiara / Kutipan Mengajar'}>
              <input
                value={form.kutipan}
            ...
            <Field label="Deskripsi Singkat (2-3 kalimat)">
              <textarea
                ...
                placeholder={staff ? 'Visi dan pengalaman kerja Anda...' : 'Visi mengajar Anda...'}
                maxLength={1000}
              />
            </Field>
          </ProfSection>
...
function KompetensiSertifikatForm({ guru, client, onSaved }) {
...
function SertifikatUploader({ client, list, onChange }) {
