...
export default function MyProfile({ onBack, guruProfile = false, kelas = null }) {
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`flex-1 ${card} p-6`}>
          {tab === 'kompetensi' ? (
            ...
            <ProfilProfesionalForm user={user} guru={guru} kelas={kelas} onSaved={setGuru} onUserSaved={setUser} />
          ) : tab === 'personal' ? (
...
function ProfilProfesionalForm({ user, guru, kelas, onSaved, onUserSaved }) {
  ...
  useEffect(() => {
    ...
    setForm({
      nama: guru.nama || '',
      jabatan: guru.jabatan || (kelas ? `Wali Kelas ${kelas.nama_kelas}` : 'Wali Kelas'),
