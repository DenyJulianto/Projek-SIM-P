...
import KegiatanFormModal from '../components/KegiatanFormModal'
import { api } from '../lib/api'

export default function KegiatanManagement({ onBack }) {
  ...
  async function handleDelete(item) {
    ...
    try {
      await api.deleteKegiatan(item.id)
      loadItems()
    } catch (err) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-navy">Kegiatan</h1>
        </div>
        <button
          onClick={openCreate}
              ...
              items.map((item) => (
                <tr key={item.id} className="border-t border-navy/5">
                  ...
                  <td className="px-4 py-3 text-navy/70">
                    ...
                    {item.tanggal_selesai ? ` - ${item.tanggal_selesai.slice(0, 10)}` : ''}
                  </td>
