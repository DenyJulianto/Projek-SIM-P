...
import PrestasiFormModal from '../components/PrestasiFormModal'
...
export default function PrestasiManagement({ onBack }) {
  ...
  async function handleDelete(item) {
    if (!window.confirm(`Hapus prestasi "${item.judul}"?`)) return
    try {
      await api.deletePrestasi(item.id)
      loadItems()
    } catch (err) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-navy">Prestasi</h1>
        </div>
        <button
          onClick={openCreate}
              ...
              items.map((item) => (
                <tr key={item.id} className="border-t border-navy/5">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gold-light/40 text-navy">
